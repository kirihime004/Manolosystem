-- =========================================================================
-- Root cause of the "column budget_approved does not exist" error
-- (migration 196 only made the error message precise enough to find
-- this): the FINANCE block of get_company_ai_context() has referenced
-- fin.budget_approved/budget_committed/budget_spent/budget_available in
-- its jsonb_build_object() since the function was first written
-- (migration 20260101000177_shared_budget_ai_coverage.sql) -- but its
-- derived table's select list never actually defined those four
-- columns, unlike the IT/HR/ADMIN/PRODUCTION blocks sitting right next
-- to it, which all follow the identical pattern correctly. This has
-- been broken since 177; it surfaced only now because nothing had
-- exercised get_company_ai_context() as a real non-bypass-RLS user
-- until the company dashboard's new AI health banner called it live.
--
-- Fix: add the same four budget subqueries (module_key = 'FINANCE',
-- matching every sibling block) to the finance derived table.
-- =========================================================================
