-- =========================================================================
-- BUDGET & PROCUREMENT ARCHITECTURE CORRECTION -- Part 4: the one hard
-- business rule -- "a department cannot use an unapproved budget for
-- procurement" -- plus closing the purchase_orders department/cost-center/
-- project gap from the schema migration.
--
-- Preparation (creating/submitting a PR against a draft budget) stays
-- allowed; the block is at the point a COMMITMENT is about to actually
-- post -- i.e. at PR-approval time, not PR-submission time. This matches
-- the spec's own stated preference exactly: "Allow draft preparation. Do
-- not allow purchase approval against an unapproved budget."
-- =========================================================================
...
-- ---------------------------------------------------------------------
-- create_purchase_order_from_pr(): now also copies department_id/
-- cost_center_id/project_id from the source PR onto the new PO, closing
-- the gap where a PO previously carried none of the three.
-- ---------------------------------------------------------------------
