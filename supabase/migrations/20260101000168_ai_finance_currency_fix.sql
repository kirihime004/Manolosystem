-- =========================================================================
-- Fix: get_finance_health_summary() and get_company_ai_context() both sum
-- customer_invoices.total/supplier_bills.total minus paid_amount -- the
-- amount in each document's OWN currency -- instead of base_currency_total,
-- which exists on both tables specifically so cross-currency aggregates
-- like this one are possible. For any company with multi-currency
-- customers/suppliers, this silently adds USD + PHP + EUR figures together
-- into one meaningless number, which the AI assistant states as fact.
--
-- Fix: use base_currency_total (falling back to total when a document was
-- never converted, i.e. already in the base currency) for the outstanding
-- total, and convert paid_amount to the base currency using the same
-- exchange_rate the document's own base_currency_total was computed with
-- (falling back to 1 for the same reason). Both tables have carried
-- exchange_rate since their original schema migrations (000079, 000082).
-- =========================================================================
