...
export default function DeliveriesListPage({ moduleKey = "IT" }: { moduleKey?: BudgetModuleKey }) {
  const { companySlug } = useParams<{ companySlug: string }>();
  const { company } = useCompany();
  const config = PROCUREMENT_MODULE_CONFIG[moduleKey];
  const { data: deliveries, isLoading } = useDeliveries(company?.id, moduleKey);

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-foreground">{config.label} Deliveries</h1>
            ...
            <TableBody>
              {deliveries.map((d) => (
                <TableRow key={d.id}>
                  ...
                  <TableCell>
                    <Link to={`/c/${companySlug}/${config.basePath}/orders/${d.purchase_order_id}`} className="font-medium text-foreground hover:underline">
