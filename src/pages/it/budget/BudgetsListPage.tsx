...
export default function BudgetsListPage() {
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    try {
      ...
      toast.success(`${budget.budget_name} created`);
      setOpen(false);
      setName(""); setTotalBudget(""); setDescription("");
    } catch (err) {
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                <div className="space-y-1.5"><Label>Budget name</Label><Input required value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. 2026 IT Budget" /></div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5"><Label>Total budget</Label><Input type="number" step="0.01" required value={totalBudget} onChange={(e) => setTotalBudget(e.target.value)} /></div>
                </div>
      ...
      <div className="rounded-lg border border-border bg-card">
        {isLoading ? (
          ...
          <Table>
            ...
            <TableBody>
              {budgets.map((b) => (
                <TableRow key={b.id} className="cursor-pointer" onClick={() => (window.location.href = `/c/${companySlug}/it/budget/budgets/${b.id}`)}>
                  <TableCell><Link to={`/c/${companySlug}/it/budget/budgets/${b.id}`} className="font-medium text-foreground hover:underline">{b.budget_name}</Link></TableCell>
