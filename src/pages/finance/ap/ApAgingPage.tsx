...
import { useApAging } from "@/features/finance/hooks";
...
export default function ApAgingPage() {
  ...
  const { data: rows, isLoading } = useApAging(company?.id);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-foreground">AP Aging</h1>
        <p className="text-sm text-muted-foreground">Outstanding supplier bills by days overdue.</p>
      </div>
      ...
      <div className="rounded-lg border border-border bg-card">
        {isLoading ? (
          ...
          <Table>
            <TableHeader><TableRow><TableHead>Supplier</TableHead><TableHead>Bill</TableHead><TableHead>Due date</TableHead><TableHead>Original</TableHead><TableHead>Paid</TableHead><TableHead>Outstanding</TableHead><TableHead>Days overdue</TableHead><TableHead>Bucket</TableHead></TableRow></TableHeader>
            <TableBody>
              {rows.map((r) => (
                <TableRow key={r.bill_id}>
                  <TableCell>{r.supplier_name}</TableCell>
                  <TableCell className="text-muted-foreground">{r.bill_number}</TableCell>
